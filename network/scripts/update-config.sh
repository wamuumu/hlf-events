...
ORIGINAL=${NETWORK_CHN_PATH}/config.json
...
function fetch_channel_config() {
    ...
    ORG=$1
    OUTPUT=${NETWORK_CHN_PATH}/config.json
    ...
    jq .data.data[0].payload.data.config ${NETWORK_CHN_PATH}/config_block.json > "${OUTPUT}"
}
...
function create_update_transaction() {
    ...
    configtxlator proto_encode \
        --input "${ORIGINAL}" \
        --type common.Config \
        --output ${NETWORK_CHN_PATH}/original_config.pb
    ...
    configtxlator compute_update \
        ...
        --original ${NETWORK_CHN_PATH}/original_config.pb \
