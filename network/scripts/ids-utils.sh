...
generate_organizations() {
    ...
    for organization in $(echo "${organizations}" | jq -r 'keys[]'); do
        ...
        jq \
            ...
            '.[$orgId] = {orgName: $orgName, orgDomain: $orgDomain}' \
...
generate_peers() {
    ...
    # Initialize the peers JSON file
    echo "{}" > ${PEERS_JSON_FILE}
    ...
    while IFS= read -r service; do
        ...
        PEER_ID=$(($(jq 'keys | length' ${PEERS_JSON_FILE}) + 1))
        jq \
            --arg peerId "$PEER_ID" \
            ...
            '.[$peerId] = {peerName: $peerName, localMspId: $localMspId, peerDomain: $peerDomain, address: $corePeerAddress, listenAddress: $listenAddress}' \
            ${PEERS_JSON_FILE} > ${PEERS_JSON_FILE}.tmp && mv ${PEERS_JSON_FILE}.tmp ${PEERS_JSON_FILE}
    done <<< "$services"
}